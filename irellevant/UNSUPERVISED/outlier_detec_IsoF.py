...
clf = IsolationForest(n_estimators=200, contamination=0.02) # ... more params?
sw = np.array(list(map(lambda x: np.log(x), X[:, 1])))
y_pred = clf.fit_predict(X)
# y_pred = clf.predict(X)


df_res = pd.DataFrame(zip(X[:,0], X[:,1], y_pred), columns=['amount', 'rowcnt', 'pred'])
print(df_res)
...
'''
Problem med Isolation Forest
- punkter samlas kring nedre mitten (färre, lägre priser)
